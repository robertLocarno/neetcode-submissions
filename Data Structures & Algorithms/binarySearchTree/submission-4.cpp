...
public:
    ...
    static std::unique_ptr<TreeNode> remove(std::unique_ptr<TreeNode> node, int key) {
        ...
        // Otherwise, this is the correct node
        if (!node->leftNode) return std::move(node->rightNode);
        if (!node->rightNode) return std::move(node->leftNode);

        std::unique_ptr<TreeNode> promoted = std::move(node->leftNode);
